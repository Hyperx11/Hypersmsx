...
from config import token,limit,channelid,webhook,prefix
...
def run():
    ...
    @bot.event
    async def on_ready():
        system("title Buffalo Store SMS BOT 67 API - https://discord.gg/znSSzHnNQj")
        system("cls")
        print(f"""
...
{Fore.LIGHTRED_EX}╚═════╝░╚═╝░░░░░╚═╝╚═════╝░  {Fore.LIGHTCYAN_EX}╚═════╝░░╚════╝░░░░╚═╝░░░
        ...
        {Fore.CYAN}[+] {Fore.LIGHTYELLOW_EX}Buffalo Store 67 Api

        {Fore.CYAN}[+] {Fore.LIGHTYELLOW_EX}Version 1.0.3

        {Fore.CYAN}[+] {Fore.LIGHTYELLOW_EX}Bot Name : {bot.user.name} 
        
        {Fore.CYAN}[+] {Fore.LIGHTYELLOW_EX}Bot ID : {bot.user.id}
        {Fore.RESET}""")
        sleep(1)
        print("ONLINE!")
    ...
    async def sms(ctx,phone,amount: int):
        ...
        if ctx.channel.id == channelid:
                if amount < limit+1:
                    post(webhook,json={"content": f"ยิงไปที่ {phone[:5]}***** โดย <@{ctx.message.author.id}>"})
                    await ctx.send(f"> กำลังยิงไปที่ ||{phone}||", delete_after=5)
            ...
            await ctx.send(f"ใช้ผิดห้องนะครับ! <@{ctx.message.author.id}> กรุณาใช้คำสั่งห้องนี้นะครับ <#{channelid}>", delete_after=5)
